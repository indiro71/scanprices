...
export default function AddProduct() {
  ...
  const addProduct = async (data) => {
    if (productData) {
      try {
        const product: IProduct = await request(
          `/scanprices/products/add/`,
          'POST',
          productData,
        );
        if (data?.alertPrice) {
  ...
  const scanProduct = async (data) => {
    if (data.url) {
      try {
        const productData = await request<IProduct>(
          `/scanprices/products/scan/`,
          'POST',
          data,
        );
    ...
    <>
      ...
      <BlockContent>
        ...
        {!loading && !productData && (
          <form
            className="form"
            onSubmit={handleSubmit(scanProduct)}
            noValidate
            autoComplete="off"
          >
            <Field>
              ...
              {productData.available ? (
                <div className="w-3/4 mx-5 p-4 shadow-lg rounded-2xl bg-white h-full">
                  <div className="w-full flex mb-3">
                    <b>
                      Price:{' '}
                      {currencyFormatter.format(productData.currentPrice, {
                        code: 'RUB',
                      })}{' '}
                    </b>
                  </div>
