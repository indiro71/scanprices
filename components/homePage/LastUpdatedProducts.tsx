...
export const LastUpdatedProducts: FC = () => {
  ...
  const fetchLastUpdatedProducts = useCallback(async () => {
    try {
      const data = await request<LastUpdatedData>(
        `/scanprices/products/lastupdated/`,
        'GET',
      );
  ...
  const tableBody =
    lastUpdatedProducts &&
    lastUpdatedProducts.map((product) => {
      ...
      return [
        ...
        <span
          className={
            moment().isSame(product.dateUpdate, 'day') ? 'text-xl' : ''
          }
        >
          {moment(product.dateUpdate).format('DD-MM-YYYY  HH:mm')}
        </span>,
        <span>
          <b className={price > 0 ? 'text-red-500' : 'text-green-500'}>
            {convertPrice(price)}
          </b>
        </span>,
      ];
